...
export class CadastroIntegrantesComponent implements OnInit {
  ...
  constructor(public integrantesService: IntegrantesService, private userService: UserService) { }
  ...
  refreshIntegrantes() {
    this.userService.getUserProfile().subscribe((usuario:any) => {
      ...
      this.integrantesService.getIntegrantes().subscribe((res: Integrantes[]) => {
        res.filter(x => !this.integrantes.map(x => x._id).includes(x._id)).forEach(integrante => {
          this.userService.getUser(integrante.user).subscribe((user: any) => {
